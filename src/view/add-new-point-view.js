...
function createAddNewPointTemplate(point, destinations) {
  const {type, basePrice, dateFrom, dateTo, destination} = point;
  ...
  function createPicturesTemplate() {
    const pictureObject = mockDestinations.find((element) => element.name === destination);

    if (pictureObject.pictures === 0) {
      return '';
    }

    const picturesArray = [];

    for (const key of pictureObject.pictures) {
      picturesArray.push(key);
    }
      ...
      <form class="event event--edit" action="#" method="post">
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--time">
            ...
            <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${humanizeEventDate(dateFrom, DATE_FORMATS.shortDateTime)}">
            &mdash;
            <label class="visually-hidden" for="event-end-time-1">To</label>
            <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${humanizeEventDate(dateTo, DATE_FORMATS.shortDateTime)}">
          </div>
